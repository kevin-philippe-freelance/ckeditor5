...
This is a major releases that introduces many smaller features, dozens of bug fixes and a couple of infrastructure changes (an upgrade to `webpack@4`, simplified structure of the build repository). Additionally, the `DrupalEditor#element` property was renamed to `DrupalEditor#sourceElement` and the `DrupalEditor#updateElement()` method was renamed to `DrupalEditor#updateSourceElement()`.
