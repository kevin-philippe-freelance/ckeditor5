...
describe( 'DrupalEditor build', () => {
	...
	describe( 'build', () => {
		it( 'contains plugins', () => {
			expect( DrupalEditor.builtinPlugins ).to.not.be.empty;
		} );

		it( 'contains config', () => {
			expect( DrupalEditor.defaultConfig.toolbar ).to.not.be.empty;
		} );
	} );

	describe( 'create()', () => {
		beforeEach( () => {
			return DrupalEditor.create( editorElement )
				.then( newEditor => {
					editor = newEditor;
				} );
		} );
		...
		it( 'creates an instance which inherits from the DrupalEditor', () => {
			expect( editor ).to.be.instanceof( DrupalEditor );
	...
	describeMemoryUsage( () => {
		testMemoryUsage(
			...
			() => DrupalEditor.create( document.querySelector( '#mem-editor' ) ) );
	} );
} );
